...
use super::anomaly::AnomalyResult;

/// Generate plain-English explanations for an anomaly result.
    ...
    if result.memory_anomaly {
        if result.memory_score > 3.0 {
            explanations.push(format!(
                "Memory usage for '{}' is extremely high compared to its normal behavior.",
                process,
            ));
        } else {
            explanations.push(format!(
                "Memory usage for '{}' is significantly higher than usual.",
                process,
            ));
        }
    }
